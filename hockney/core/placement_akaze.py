"""
placement_akaze.py — AKAZE-based reference placement engine.

AKAZE (Accelerated-KAZE) uses nonlinear diffusion filtering to build a
scale space that preserves object boundaries — unlike SIFT/ORB which use
Gaussian blurring that smears edges.  This makes AKAZE better at:
  - Blurry or soft-focus images
  - Textured surfaces with subtle variation
  - Mixed sharp/soft regions in the same image

Built into OpenCV — zero download, zero dependencies beyond cv2.
Uses binary descriptors (like ORB) with BFMatcher + Hamming distance.
Slower than ORB but significantly more accurate.
"""
...
def extract_akaze(img_rgb: np.ndarray) -> tuple[list, Optional[np.ndarray]]:
    """Extract AKAZE keypoints and descriptors from an RGB image."""
    grey = cv2.cvtColor(img_rgb, cv2.COLOR_RGB2GRAY)
    akaze = cv2.AKAZE_create()
    kp, desc = akaze.detectAndCompute(grey, None)
    return kp, desc


def match_akaze(desc_a: np.ndarray, desc_b: np.ndarray,
                ratio: float = LOWE_RATIO) -> list[cv2.DMatch]:
    """BFMatcher with Hamming distance + Lowe's ratio test for AKAZE."""
...
def place_with_akaze(
    ...
    """
    AKAZE-based reference placement.
    ...
    # ── 1. Extract AKAZE from references ───────────────────────────────
    ref_kp = {}
    ref_desc = {}
    ref_sizes = {}
    ...
    for ref in refs:
        ...
        try:
            ...
            arr = np.array(pil)
            kp, desc = extract_akaze(arr)
            if desc is not None and len(kp) > 0:
                ref_kp[ref.slot] = kp
                ref_desc[ref.slot] = desc
                log.info("AKAZE ref %s: %d keypoints", ref.slot, len(kp))
        except Exception as e:
            log.warning("AKAZE ref extraction failed %s: %s", ref.slot, e)
    ...
    # ── 2. Extract AKAZE from details at multiple scales ───────────────
    detail_multiscale = {}
    ...
    for record in records:
        ...
        try:
            ...
            for ds in DETAIL_SCALES:
                ...
                kp, desc = extract_akaze(resized)
                if desc is not None and len(kp) > 0:
                    scales_data.append((ds, kp, desc, (new_w, new_h)))

            detail_multiscale[record.id] = scales_data
        except Exception as e:
            log.warning("AKAZE detail extraction failed %s: %s", record.id[:6], e)
        done += 1
        if progress_cb:
            progress_cb(int(done / total_work * 100))
    ...
    for record in records:
        ...
        for slot in ref_kp:
            for ds, d_kp, d_desc, (dw, dh) in detail_multiscale[record.id]:
                try:
                    good = match_akaze(ref_desc[slot], d_desc, ratio=LOWE_RATIO)
                    if len(good) < 4:
                        continue
                    ...
                    log.debug("AKAZE match %s<>%s@%.0f%% failed: %s",
        ...
        if best_slot and best_ref_centroid:
            ...
            placed_count += 1
            log.info("AKAZE placed %s in %s (%d inliers, scale=%.0f%%)",
